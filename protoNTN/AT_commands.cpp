...
bool ATcommands::sendCommand(char *command, uint16_t timeout)
{
  while (mySerial->available())
    Serial.print((char)mySerial->read()); // Clear input buffer
...
bool ATcommands::sendCommand(char *command, char *expect, char *reply, uint16_t timeout)
{
  ...
  while (!replyMatch && millis() - timer < timeout)
  {
    if (mySerial->available())
    {
      ...
      {
        Serial.print("..avant : ");
        Serial.print((uint8_t)recu);
        replybuffer[idx] = "A";
        Serial.print("..apres : ");
        Serial.print((uint8_t)recu);
      }*/

      replybuffer[idx] = recu;

      /*Serial.print(replybuffer[idx]);
      Serial.print("(");
      Serial.print((uint8_t)replybuffer[idx]);
      Serial.print(")");*/
      idx++;
